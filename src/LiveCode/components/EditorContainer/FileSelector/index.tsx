...
import { ENTRY_FILE_NAME, IMPORT_MAP_FILE_NAME, MAIN_FILE_NAME } from '@/LiveCode/files'
import { LiveCodeContext } from '@/LiveCode/LiveCodeContext'
import type { IFileSelector } from '@/LiveCode/types'
...
export const FileSelector: React.FC<IFileSelector> = (props) => {
  ...
  const handleValidateTab = (newName: string, oldName: string) => {
    if (!/\.(jsx|tsx|js|ts|css|json)$/.test(newName)) {
      onError('LiveCode only supports *.jsx, *.js, *.css, *.json files.')
      return false
    }
